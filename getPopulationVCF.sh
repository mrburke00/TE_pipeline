...
while read sra_example;	do
	...
	#gzip -d  $data_dir/output/"${sra_example}"/out.pass.vcf.gz
        ...
        bcftools query -f '%CHROM\t%POS\t%INFO/END\t%SVINSSEQ\t'${sra_example}'\n' output/"${species}"/peak_"$low"-"$high"/output_seqs.vcf >> output/"${species}"/peak_"$low"-"$high"/"${species}"_"$low"-"$high"_pop_vcf.txt
